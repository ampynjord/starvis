...
export const DT_NAMES: Record<number, string> = {
  ...
  [DT_ENUM]: 'ENUM',
  [DT_CLASS]: 'CLASS',
  [DT_STRONG_PTR]: 'STRONG_PTR',
  [DT_WEAK_PTR]: 'WEAK_PTR',
...
export interface DataForgeData {
  header: {
    ...
    int8ValueCount: number;
    int16ValueCount: number;
    int32ValueCount: number;
    int64ValueCount: number;
    uint8ValueCount: number;
    uint16ValueCount: number;
    uint32ValueCount: number;
    uint64ValueCount: number;
    singleValueCount: number;
    doubleValueCount: number;
    guidValueCount: number;
    stringIdValueCount: number;
    localeValueCount: number;
    enumValueCount: number;
    strongValueCount: number;
    weakValueCount: number;
    referenceValueCount: number;
...
export function readInstance(
  ...
  for (const prop of allProps) {
    if (prop.conversionType === 0) {
      ...
      pos += 4;
      const firstIndex = buf.readUInt32LE(pos);
      pos += 4;
      const arr: any[] = [];
...
function readValueInline(
  ...
  switch (dt) {
    ...
    case DT_INT8:
      return [buf.readInt8(pos), pos + 1];
    case DT_INT16:
      return [buf.readInt16LE(pos), pos + 2];
    case DT_INT32:
      return [buf.readInt32LE(pos), pos + 4];
    case DT_INT64:
      return [Number(buf.readBigInt64LE(pos)), pos + 8];
    case DT_UINT8:
      return [buf.readUInt8(pos), pos + 1];
    case DT_UINT16:
      return [buf.readUInt16LE(pos), pos + 2];
    case DT_UINT32:
      return [buf.readUInt32LE(pos), pos + 4];
    case DT_UINT64:
      return [Number(buf.readBigUInt64LE(pos)), pos + 8];
    case DT_STRING: {
      const strOff = buf.readUInt32LE(pos);
      return [dfData.stringTable1.get(strOff) ?? `STR_${strOff}`, pos + 4];
    }
    case DT_SINGLE:
      return [Math.round(buf.readFloatLE(pos) * 1e6) / 1e6, pos + 4];
    case DT_DOUBLE:
    ...
    case DT_GUID:
      return [readGuidAt(buf, pos), pos + 16];
    case DT_ENUM: {
      ...
      if (depth < maxDepth) {
        for (const np of nestedProps) {
          if (np.conversionType === 0) {
            ...
            const cnt = buf.readUInt32LE(curPos);
            curPos += 4;
            const fi = buf.readUInt32LE(curPos);
            curPos += 4;
            const arr: any[] = [];
...
function readValueAtIndex(
  ...
  switch (dt) {
    case DT_BOOLEAN:
      return buf.readUInt8(va.boolean + index) !== 0;
    case DT_INT8:
      return buf.readInt8(va.int8 + index);
    case DT_INT16:
      return buf.readInt16LE(va.int16 + index * 2);
    case DT_INT32:
      return buf.readInt32LE(va.int32 + index * 4);
    case DT_INT64:
      return Number(buf.readBigInt64LE(va.int64 + index * 8));
    case DT_UINT8:
      return buf.readUInt8(va.uint8 + index);
    case DT_UINT16:
      return buf.readUInt16LE(va.uint16 + index * 2);
    case DT_UINT32:
      return buf.readUInt32LE(va.uint32 + index * 4);
    case DT_UINT64:
      return Number(buf.readBigUInt64LE(va.uint64 + index * 8));
    case DT_STRING: {
      const strOff = buf.readUInt32LE(va.stringId + index * 4);
      return dfData.stringTable1.get(strOff) ?? '';
    }
    case DT_SINGLE:
      return Math.round(buf.readFloatLE(va.single + index * 4) * 1e6) / 1e6;
    case DT_DOUBLE:
      return buf.readDoubleLE(va.double + index * 8);
    case DT_LOCALE: {
      const locOff = buf.readUInt32LE(va.locale + index * 4);
      return dfData.stringTable1.get(locOff) ?? '';
    }
    case DT_GUID:
      return readGuidAt(buf, va.guid + index * 16);
    case DT_ENUM: {
...
export function parseDataForge(buf: Buffer): DataForgeData {
  ...
  logger.debug(`Data section: offset=${dataOffset}, size=${lastOff}, endCheck=${dataOffset + lastOff === buf.length ? 'OK' : 'MISMATCH'}`, {
    module: 'dataforge',
  });
