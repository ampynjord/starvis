  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-600 font-mono-sc">External REST API · Beta access required</p>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-sm border border-amber-800/40 bg-amber-950/10 px-4 py-3">
        ...
        <p className="font-rajdhani text-sm text-slate-400">
          The external API is accessible to <span className="text-amber-400 font-semibold">beta tester</span> and{' '}
      ...
      <Section title="API Documentation" icon={BookOpen}>
        <p className="font-rajdhani text-sm text-slate-400 leading-relaxed">
          The full API reference is available via Swagger UI — interactive documentation with all endpoints,
      ...
      <Section title="Generate an API token" icon={Key}>
        <p className="font-rajdhani text-sm text-slate-400 leading-relaxed">
          API tokens are long-lived JWTs (1 year validity) tied to your account. Go to your profile to generate one.
...
Authorization: Bearer YOUR_API_TOKEN`}
      ...
      <Section title="Base URL & versioning" icon={Zap}>
        <p className="font-rajdhani text-sm text-slate-400">
          All game data endpoints are versioned under <code className="font-mono text-cyan-400">/api/v1/</code>.
        </p>
        <Code>{`https://starvis.gg/api/v1/ships
https://starvis.gg/api/v1/components
https://starvis.gg/api/v1/manufacturers
https://starvis.gg/api/v1/items
https://starvis.gg/api/v1/commodities
https://starvis.gg/api/v1/missions
https://starvis.gg/api/v1/locations        # beta only
https://starvis.gg/api/v1/search?q=aurora`}
        </Code>
      </Section>
  ...
  "https://starvis.gg/api/v1/ships?limit=20&page=1"`}
        </Code>
        <p className="font-rajdhani text-sm text-slate-400">Example response structure:</p>
        <Code>{`{
  "total": 412,
  "page": 1,
  "limit": 20,
  "data": [
    {
      ...
      <Section title="Rate limits" icon={Code2}>
        ...
        <p className="font-rajdhani text-sm text-slate-500">
          Note: the API and game data are in early access. Endpoints, schemas and rate limits may evolve.
          Check the changelog or the Swagger docs for updates.
        </p>
      </Section>
    </div>
  );
