...
function mockShip(overrides: Record<string, unknown> = {}): Row {
  return {
    ...
    scm_speed: 210,
    max_speed: 1235,
    boost_speed_forward: 800,
    boost_speed_backward: 200,
    pitch_max: 65,
    yaw_max: 65,
    roll_max: 130,
    hydrogen_fuel_capacity: 20000,
    quantum_fuel_capacity: 583,
    armor_physical: 0.5,
    armor_energy: 0.6,
    armor_distortion: 0.8,
    armor_thermal: 0.7,
    armor_signal_ir: 4200,
    armor_signal_em: 3100,
    armor_signal_cs: 1800,
    cross_section_x: 12.5,
    cross_section_y: 6.0,
    cross_section_z: 18.0,
    ship_matrix_id: 1,
...
function mockWeapon(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 1,
    port_name: 'hardpoint_weapon_gun_01',
    port_type: 'WeaponGun',
    component_uuid: 'comp-w1',
    component_class_name: 'KLWE_LaserRepeater_S3',
    type: 'WeaponGun',
    ...
    weapon_dps: 280,
    weapon_burst_dps: 310,
    weapon_sustained_dps: 250,
    weapon_damage: 42,
    weapon_fire_rate: 6.67,
    weapon_range: 3600,
    weapon_damage_physical: 0,
    weapon_damage_energy: 42,
    weapon_damage_distortion: 0,
    power_draw: 3.5,
    heat_generation: 2.8,
    ...overrides,
    constructor: { name: 'RowDataPacket' },
...
function mockShield(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 10,
    port_name: 'hardpoint_shield_01',
    port_type: 'Shield',
    component_uuid: 'comp-s1',
    component_class_name: 'GAMA_Shield_S2',
    type: 'Shield',
    name: 'S02 FR-66 Shield',
    size: 2,
    grade: 'A',
    manufacturer_code: 'GAMA',
    shield_hp: 5500,
    shield_regen: 55,
    shield_regen_delay: 6.5,
...
function mockPowerPlant(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 20,
    port_name: 'hardpoint_power_plant_01',
    port_type: 'PowerPlant',
    component_uuid: 'comp-pp1',
    component_class_name: 'AEGS_PowerPlant_S2',
    type: 'PowerPlant',
    name: 'S02 Regulus Power Plant',
    size: 2,
    grade: 'B',
...
function mockCooler(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 30,
    port_name: 'hardpoint_cooler_01',
    port_type: 'Cooler',
    component_uuid: 'comp-c1',
    component_class_name: 'AEGS_Cooler_S2',
    type: 'Cooler',
    name: 'S02 NDB-30 Cooler',
    size: 2,
    grade: 'B',
    manufacturer_code: 'AEGS',
    cooling_rate: 280000,
...
function mockQD(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 40,
    port_name: 'hardpoint_quantum_drive_01',
    port_type: 'QuantumDrive',
    component_uuid: 'comp-qd1',
    component_class_name: 'AEGS_QuantumDrive_S1',
    type: 'QuantumDrive',
    name: 'S01 Expedition Quantum Drive',
    size: 1,
    grade: 'A',
    manufacturer_code: 'AEGS',
    qd_speed: 77000000,
    ...
    qd_range: 1.2e8,
    qd_tuning_rate: 0.2,
    qd_alignment_rate: 0.15,
    qd_disconnect_range: 500,
    power_draw: 2.0,
    heat_generation: 1.5,
    ...overrides,
    constructor: { name: 'RowDataPacket' },
...
function mockMissile(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 50,
    port_name: 'hardpoint_missile_01',
    port_type: 'Missile',
    component_uuid: 'comp-m1',
    component_class_name: 'BEHR_Missile_S2',
    type: 'Missile',
    name: 'S02 Dominator II Missile',
    size: 2,
    missile_damage: 3200,
    missile_speed: 1100,
    missile_range: 5000,
    missile_lock_time: 2.5,
    missile_signal_type: 'IR',
    missile_damage_physical: 2200,
    missile_damage_energy: 1000,
    missile_damage_distortion: 0,
    power_draw: 0,
    heat_generation: 0,
    ...overrides,
    constructor: { name: 'RowDataPacket' },
...
function mockCM(overrides: Record<string, unknown> = {}): Row {
  return {
    id: 60,
    port_name: 'hardpoint_cm_01',
    port_type: 'Countermeasure',
    component_uuid: 'comp-cm1',
    type: 'Countermeasure',
    name: 'CML Flare Launcher',
    cm_ammo_count: 24,
    power_draw: 0.5,
    heat_generation: 0.3,
    ...overrides,
...
describe('aggregateLoadoutStats', () => {
  ...
  describe('Thermal balance', () => {
    it('calculates heat generation vs cooling rate', () => {
      const loadout = [mockCooler({ cooling_rate: 280000 }), mockWeapon({ heat_generation: 2.8, weapon_dps: 200 })];
      const stats = service.aggregateLoadoutStats(loadout, ship, 12, 6, 18) as any;
  ...
  describe('Utility weapons (mining/salvage/tractor)', () => {
    it('classifies mining lasers as utility', () => {
      const loadout = [
        mockWeapon({
          name: 'Arbor Mining Laser MH1',
          class_name: 'GRLN_MiningLaser_S1',
          weapon_dps: 50,
          weapon_damage: 15,
        }),
      ];
    ...
    it('classifies tractor beams as utility', () => {
      const loadout = [mockWeapon({ name: 'MaxOx Tractor', class_name: 'MOOX_TractorBeam_S1', weapon_dps: 0 })];
      const stats = service.aggregateLoadoutStats(loadout, ship, 12, 6, 18) as any;
      expect(stats.utility.count).toBe(1);
