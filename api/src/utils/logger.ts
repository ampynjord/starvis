...
if (!existsSync(LOG_DIR)) {
  try {
    ...
    console.warn(`[logger] Could not create log dir "${LOG_DIR}": ${(e as Error).message}`);
  }
}
...
const logger = winston.createLogger({
  level: LOG_LEVEL,
  format: winston.format.combine(winston.format.timestamp({ format: 'HH:mm:ss' }), winston.format.errors({ stack: true })),
  transports,
});
