...
import {
    ...
    RELEVANT_TYPES, UTILITY_WEAPON_RX,
    cleanName, detectUtilityType, int, num, r1, r2, r4, r6,
} from "./shared.js";
...
export class LoadoutService {
  ...
  private buildHardpoints(loadout: Row[]): Record<string, unknown>[] {
    ...
    for (const root of rootPorts) {
      ...
      if (hasRelevantChildren || (mountType && children.length > 0)) {
        hardpoints.push({
          port_id: root.id, port_name: portName, display_name: this.cleanPortName(portName),
          category, port_min_size: int(root.port_min_size) || null, port_max_size: effectiveMaxSize,
          mount_type: mountType, mount_class_name: compCls || null, mount_size: mountSize,
          component: null, items, swapped: !!root._swapped,
        });
      } else if (root.component_uuid && (root.type || componentType)) {
        hardpoints.push({
          port_id: root.id, port_name: portName, display_name: this.cleanPortName(portName),
          category, port_min_size: int(root.port_min_size) || null, port_max_size: effectiveMaxSize,
          mount_type: null, mount_class_name: null, mount_size: null,
          component: this.buildComponentInfo(root, childMap), items: [], swapped: !!root._swapped,
        });
      }
    }
  ...
  private cleanMountName(className: string): string {
    ...
    name = name.replace(/[Ss]\d+/g, "").replace(/_+/g, " ").replace(/\s+/g, " ").trim();
    if (!name) {
  ...
  private buildComponentInfo(row: Row, childMap: Map<number, Row[]>): Record<string, unknown> {
    ...
    const subItems = relevantSubChildren.map(c => {
      ...
      return {
        port_id: c.id, port_name: c.port_name,
        port_max_size: int(c.port_max_size) || null, port_min_size: int(c.port_min_size) || null,
        ...
        type: cType, sub_type: c.sub_type || null,
        size: int(c.size) || (cMountSize ? parseInt(cMountSize) : null),
        grade: c.grade || null, manufacturer_code: c.manufacturer_code || null,
        hp: r2(num(c.hp)) || null,
        ...(cType === "WeaponGun" && {
          weapon_dps: r2(num(c.weapon_dps)) || null, weapon_burst_dps: r2(num(c.weapon_burst_dps)) || null,
          weapon_sustained_dps: r2(num(c.weapon_sustained_dps)) || null, weapon_fire_rate: r2(num(c.weapon_fire_rate)) || null,
          weapon_range: Math.round(num(c.weapon_range)) || null, power_draw: r2(num(c.power_draw)) || null,
        }),
        ...(cType === "Missile" && {
          missile_damage: r2(num(c.missile_damage)) || null, missile_signal_type: c.missile_signal_type || null,
          missile_speed: r2(num(c.missile_speed)) || null, missile_range: Math.round(num(c.missile_range)) || null,
        }),
        ...(cType !== "WeaponGun" && cType !== "Missile" && {
          weapon_dps: num(c.weapon_dps) || null, weapon_range: num(c.weapon_range) || null, missile_damage: num(c.missile_damage) || null,
        }),
        swapped: !!c._swapped,
      };
    });
    ...
    return {
      port_id: row.id, port_name: row.port_name,
      ...
      type: effectiveType, size: int(row.size) || (mountSize ? parseInt(mountSize) : null),
      port_max_size: int(row.port_max_size) || null, port_min_size: int(row.port_min_size) || null,
      grade: row.grade || null, manufacturer_code: row.manufacturer_code || null,
      hp: r2(num(row.hp)) || null, power_draw: r2(num(row.power_draw)) || null, heat_generation: r2(num(row.heat_generation)) || null,
      ...(type === "WeaponGun" && !isUtility && {
        weapon_dps: r2(num(row.weapon_dps)) || null, weapon_burst_dps: r2(num(row.weapon_burst_dps)) || null,
        weapon_sustained_dps: r2(num(row.weapon_sustained_dps)) || null, weapon_fire_rate: r2(num(row.weapon_fire_rate)) || null,
      ...
      ...(type === "QuantumDrive" && {
        qd_speed: r2(num(row.qd_speed)) || null, qd_spool_time: r2(num(row.qd_spool_time)) || null,
        qd_cooldown: r2(num(row.qd_cooldown)) || null, qd_fuel_rate: r2(num(row.qd_fuel_rate)) || null, qd_range: r2(num(row.qd_range)) || null,
      }),
      ...(type === "Missile" && {
        missile_damage: r2(num(row.missile_damage)) || null, missile_signal_type: row.missile_signal_type || null,
        missile_speed: r2(num(row.missile_speed)) || null, missile_range: Math.round(num(row.missile_range)) || null,
      ...
      ...(type === "QuantumInterdictionGenerator" && { qig_jammer_range: r2(num(row.qig_jammer_range)) || null, qig_snare_radius: r2(num(row.qig_snare_radius)) || null }),
