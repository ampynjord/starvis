    ...
    try {
      const uexRows = await prisma.$queryRawUnsafe<Row[]>(
        toPostgres(`SELECT
              ...
              p.entity_uuid as component_uuid,
              NULL::text as component_class_name,
              'uex_uuid' as match_type,
       ...
       WHERE p.env = ? AND p.entity_kind = 'component' AND p.entity_uuid = ? AND p.is_available = TRUE
