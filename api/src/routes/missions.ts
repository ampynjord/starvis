...
export function mountMissionRoutes(router: Router, deps: RouteDependencies): void {
  ...
  /**
   * GET /api/v1/missions/filters
   */
  router.get(
    '/api/v1/missions/filters',
    requireGameData,
    asyncHandler(async (req, res) => {
      const env = getQueryString(req, 'env') ?? 'live';
      const result = await gameDataService!.missions.getMissionFilters(env);
   ...
   * Lists all distinct factions referenced in missions
   */
