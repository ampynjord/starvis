...
describe('source-adapters (HTTP)', () => {
  it('returns empty maps when no env vars are set', async () => {
  ...
  it('loads community payload from URL and maps snake_case fields', async () => {
    ...
    expect(component?.confidenceScore).toBe(92);

    await server.close();
  });

  it('merges payloads preferring higher confidence_score', async () => {
    const server = await startJsonServer((_req, res) => {
      res.setHeader('content-type', 'application/json');
      res.end(
        JSON.stringify({
          items: [
            { class_name: 'ITEM_A', name: 'Low confidence', confidence_score: 50 },
            { class_name: 'ITEM_A', name: 'High confidence', confidence_score: 90 },
          ],
        }),
      );
    });
    ...
    expect(data.items.get('ITEM_A')?.name).toBe('High confidence');

    await server.close();
  });

  it('returns empty when URL returns non-JSON', async () => {
    const server = await startJsonServer((_req, res) => {
