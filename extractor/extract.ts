 ...
 *
 * Standalone tool that reads Star Citizen's P4K file locally, extracts game
 * data, and writes it to PostgreSQL.
 */
...
import { Pool } from 'pg';
import { formatModules, VALID_MODULES } from './src/cli/modules.js';
...
config({ path: resolve(import.meta.dirname, getEnvFileFromArgv(process.argv.slice(2))) });
...
async function main() {
  const cli = parseCliOptions(process.argv.slice(2), pkg.version);
  configureLogger({
    level: cli.logLevel,
...
main().catch((error) => {
  ...
  if (existsSync(resolve(import.meta.dirname, 'package.json'))) {
