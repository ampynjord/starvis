...
export class ExtractionService {
  ...
  private async _doExtractAll(onProgress?: (msg: string) => void, options: ExtractionOptions = DEFAULT_OPTIONS): Promise<ExtractionStats> {
    ...
    try {
      ...
      if (run('game-insights')) {
        ...
        stats.gameInsights = await this.saveGameInsights(conn, env, onProgress);
      }
  ...
  private async saveGameInsights(conn: PoolClient, env: GameEnv, onProgress?: (msg: string) => void): Promise<number> {
    const records = extractGameInsights(this.df, onProgress);
    if (!records.length) {
    ...
    await this.saveGameInsightDerivedData(conn, env, derived, onProgress);
    return saved;
  }

  private async saveGameInsightDerivedData(
    conn: PoolClient,
    env: GameEnv,
    data: DerivedGameInsightData,
    onProgress?: (msg: string) => void,
  ): Promise<void> {
