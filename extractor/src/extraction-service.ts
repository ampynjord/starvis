...
export interface ExtractionOptions {
  ...
  env: GameEnv;
  /** MySQL pool connected to rsi_website DB — required for galactapedia/comm-links/starmap/ship-matrix modules */
  rsiPool?: Pool;
}
...
export class ExtractionService {
  ...
  private async saveComponents(conn: PoolConnection, onProgress?: (msg: string) => void): Promise<number> {
    ...
    const COMP_UPDATE = `ON DUPLICATE KEY UPDATE
            ...
            canonical_component_key=new.canonical_component_key,
            type=new.type,
  ...
  private async saveShopsData(conn: PoolConnection, onProgress?: (msg: string) => void): Promise<{ shops: number; inventory: number }> {
    ...
    const [locRows]: any = await conn.execute(
      `SELECT class_name, name, loc_key, \`type\` FROM locations WHERE loc_key IS NOT NULL AND loc_key != ''`,
    );
    ...
    if (shopRows.length > 0) {
      savedShops = await ExtractionService.batchUpsert(
        conn,
        `INSERT INTO shops (name, normalized_name, canonical_shop_key, canonical_location_key, location, \`system\`, planet_moon, city, shop_type, class_name) VALUES`,
        `ON DUPLICATE KEY UPDATE
           name=new.name,
           normalized_name=new.normalized_name,
  ...
  private async saveShipCtmModels(conn: PoolConnection, onProgress?: (msg: string) => void): Promise<void> {
    ...
    const [rows] = await conn.execute<any[]>(
      `SELECT s.class_name, s.name, sm.url as rsi_url
       FROM ships s
       INNER JOIN rsi_website.ship_matrix sm ON s.ship_matrix_id = sm.id
