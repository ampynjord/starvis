...
|---|---|---|
| P4K / DataForge | Ships, components, FPS items, commodities, extracted shop locations/franchises, early-access inferred shop inventory, missions, mining, crafting, locations, paints and extended game insights for loot, reputation, factions, navigation, environments, services, medical data, FPS details, ammo and inventory containers. Shop inventory rows expose `source` and `confidence`; prices and rentals stay empty until a reliable extracted source is mapped. The IHM consumes normalized insight data through page-named API routes such as factions, ammo, armor, utility and blueprints. | `extractor/` CLI. |
...
`/intel` lists the bot capabilities and dataset counters directly in Discord. `/starvis` uses the STARVIS AI endpoint for free-form questions and suggests a specialized command when structured data is a better fit. `/shop` searches extracted shop locations and franchises; inventory is currently early-access inferred by shop type and includes confidence metadata when exposed by the API.
